...
public interface UserRepo extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT DISTINCT u FROM User u " +
            ...
            "AND u.isActive = true " +
            // Kiểm tra Bác sĩ có lịch làm việc tại Clinic này không
            "AND EXISTS (SELECT s FROM DoctorSchedule s WHERE s.doctor.id = u.id AND s.clinic.id = :clinicId) " +
            // LOGIC QUAN TRỌNG: Đếm số lượng chuyên khoa khớp
            // Nếu danh sách yêu cầu có 2 chuyên khoa, bác sĩ phải có đủ 2 cái đó.
    ...
    List<User> findDoctorsBySpecialties(
            ...
            @Param("specialties") List<String> specialties, // Danh sách tên chuyên khoa
            @Param("count") Long count // Số lượng chuyên khoa cần tìm (size của list)
    );
}
