...
public class ScheduleValidationServiceImpl implements ScheduleValidationService {
        ...
        for (int dayIndex = 0; dayIndex < 6; dayIndex++) {
            ...
            if (clinicIdsInDay.size() != 2) {
                if (clinicIdsInDay.isEmpty()) {
                    ...
                    Integer clinicId = clinicIdsInDay.iterator().next();
                    Clinic clinic = clinicsCache.get(clinicId);
                    String clinicName = clinic != null ? clinic.getClinicName() : "Clinic ID " + clinicId;
                    result.addError("Only 1 clinic (" + clinicName + ") is active on " + dayName + ". Both clinics must be active.");
                } else {
                    result.addError("Too many clinics (" + clinicIdsInDay.size() + ") are active on " + dayName + ". Exactly 2 clinics must be active.");
                }
                continue;
            }
            ...
            for (CreateWeeklyScheduleRequest.DoctorAssignmentRequest assignment : dayAssignments) {
                ...
                if (!validateAssignment(assignment, result, dayName, doctorsCache, clinicsCache, roomsCache, doctorSpecialtiesCache, userRolesCache, validatedDoctors, doctorDayKey)) {
                    continue;
                }
            ...
            for (Integer clinicId : clinicIdsInDay) {
                ...
                if (morningDoctors.size() < 2) {
                    result.addError(clinicName + " must have at least 2 doctors assigned in the morning shift (08:00-11:00) on " + dayName + ". Found " + morningDoctors.size() + " doctor(s)");
                }

                Set<Integer> afternoonDoctors = shiftDoctors.getOrDefault("afternoon", new HashSet<>());
                if (afternoonDoctors.size() < 2) {
                    result.addError(clinicName + " must have at least 2 doctors assigned in the afternoon shift (13:00-18:00) on " + dayName + ". Found " + afternoonDoctors.size() + " doctor(s)");
                }
            }
            ...
            for (String specialty : allSpecialties) {
                ...
                if (clinicDoctors == null || clinicDoctors.size() != 2) {
                    result.addError("Specialty \"" + specialty + "\" must have doctors assigned to both clinics on " + dayName + ". Found in " + (clinicDoctors != null ? clinicDoctors.size() : 0) + " clinic(s)");
                } else {
                    for (Integer clinicId : clinicIdsInDay) {
                        if (!clinicDoctors.containsKey(clinicId) || clinicDoctors.get(clinicId).isEmpty()) {
                            Clinic clinic = clinicsCache.get(clinicId);
                            String clinicName = clinic != null ? clinic.getClinicName() : "Clinic ID " + clinicId;
                            result.addError("Specialty \"" + specialty + "\" must have at least one doctor assigned to " + clinicName + " on " + dayName);
                        }
                    }
                }
            }
    ...
    private boolean validateAssignment(
        ...
        if (!Boolean.TRUE.equals(doctor.getIsActive())) {
            if (!alreadyValidated) {
                result.addError("Doctor ID " + assignment.getDoctorId() + " (" + doctor.getFullName() + ") is inactive and cannot be assigned to schedule on " + dayName);
                validatedDoctors.add(doctorDayKey);
            }
            return false;
        }
        ...
        if (clinic.getIsActive() != null && !Boolean.TRUE.equals(clinic.getIsActive())) {
            result.addError("Clinic ID " + assignment.getClinicId() + " (" + clinic.getClinicName() +
                    ") is inactive on " + dayName);
            return false;
        }
